                ...
                >
                  ...
                  <tbody>
                    <tr v-for="payment in this.payments" :key="payment.id">
                        ...
                        <th>{{payment.name}}</th>
                        <th>{{payment.cost}}</th>
                        <th v-if="payment.isPay" style="color: green">Đã Thanh Toán</th>
                        <th v-else-if="!payment.isPay" style="color: red">Chưa Thanh Toán</th>
                    </tr>
                  </tbody>
                </table>
              </div>
...
export default {
  ...
  methods: {
    ...
    async getListPaymentByRoomId() {
      this.showNextBtn = false
      this.showPreviousBtn = false
      let token = this.$store.state.token;
      await axios
      .get(`/api/paymentbyroom/${this.roomid}/`, {
        headers: {
    ...
    async getRoomId() {
        ...
        await axios.get(`/api/room-user/`, {
            headers: {
                'Authorization': `Bearer ${token}`,
                "Content-Type": "application/json",
            },
        }).then(resp => {
            this.roomid = resp.data.find(el => el.user === parseInt(localStorage.getItem('user'))).id
            this.getListPaymentByRoomId()
        }).catch(err => console.log(err))
    }
  },
  mounted() {
