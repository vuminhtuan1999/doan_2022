  ...
  methods: {
    ...
    addNewRoom() {
      let formData = {
        ...
        number_seat: parseInt(this.room.number_seat),
        room_position: this.room.room_position,
      }

      let token = this.$store.state.token
      axios.post(`/api/study-room/`, formData, {
        headers: {
    ...
    async getListRoom() {
      ...
      await axios
        .get(`/api/study-room/?page=${this.currentPage}&size=${this.page_size}&search=${this.query}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            "Content-Type": "application/json",
          },
